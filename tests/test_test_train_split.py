...
testdf = pd.DataFrame({'user_id': [1, 2, 5, 5, 5, 6, 5],
                      'item_id': [2, 4, 6, 8, 62, 62, 100],
                       'rating': [1, 0, 3, 4, 5, 2, 1]})
test_sparse_item_user, test_sparse_user_item = create_sparse_matrix(testdf)
test_train_data, test_test_data, test_users_altered = test_train_split(
...
class TestTrainSplit(unittest.TestCase):
    ...
    def test_dim_drop_zeros(self):
        # should return a csr matrix with dimensions
    ...
    def test_dim_match(self):
        self.assertEqual(test_test_data.get_shape(),
                         test_train_data.get_shape())

    # make sure some users were altered in the creation of the training set
